...
class SASTPostProcess(object):
    ...
    def expand_poly_along_width(self, poly, shrink_ratio_of_width=0.3):
        ...
        left_quad = np.array([poly[0], poly[1], poly[-2], poly[-1]],
                             dtype=np.float32)
    ...
    def estimate_sample_pts_num(self, quad, xy_text):
        ...
        dense_xy_center_line = xy_text[np.linspace(0,
                                                   xy_text.shape[0] - 1,
    ...
    def detect_sast(self,
        ...
        for instance_idx in range(1, instance_count):
            ...
            xy_center_line = xy_text[np.linspace(0,
                                                 xy_text.shape[0] - 1,
                                                 sample_pts_num,
                                                 endpoint=True,
                                                 dtype=np.float32).astype(
            ...
            for x, y in xy_center_line:
                ...
                if offset_expand != 1.0:
                    offset_length = np.linalg.norm(offset,
                                                   axis=1,
                                                   keepdims=True)
                    expand_length = np.clip(offset_length * (offset_expand - 1),
                                            a_min=0.5,
                                            a_max=3.0)
            ...
            detected_poly[:, 0] = np.clip(detected_poly[:, 0],
                                          a_min=0,
                                          a_max=src_w)
            detected_poly[:, 1] = np.clip(detected_poly[:, 1],
                                          a_min=0,
                                          a_max=src_h)
